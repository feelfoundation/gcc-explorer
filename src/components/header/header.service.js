...
const HeaderConstructor = function ($rootScope) {
	...
	this.updatePriceTicker = (res) => {
		...
		// When ticker for user-stored currency is not available - switch to GCC temporarily
		if ($rootScope.currency.symbol !== 'GCC' &&
			(!$rootScope.currency.tickers ||
			!$rootScope.currency.tickers.GCC ||
			!$rootScope.currency.tickers.GCC[$rootScope.currency.symbol])) {
			$rootScope.currency.symbol = 'GCC';
		}
	};
