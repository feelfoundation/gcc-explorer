Feature: Address page
  Scenario: should show title, summary, and transactions
    ...
    And I should see table "summary" containing:
      ...
      | Total balance | 99,675,450.108366 GCC                                            |
      | Transactions  | 2 1329                                                           |
    ...
    And I should see table "transactions table" with 20 rows starting with:
      ...
      |----------------|---------------------------|-----------------------|--------------------------|-----------|---------|---------------|
      | /\d{18,20}/    | /2017\/06\/16 \d\d:09:08/ | 16313739661670634666L | 14989416087250274755L    | 1,000 GCC | 0.1 GCC | Confirmed     |
      | /\d{18,20}/    | /2017\/06\/16 \d\d:09:08/ | 16313739661670634666L | /standby_\d{3}\|\d{20}L/ | 1,000 GCC | 0.1 GCC | Confirmed     |
      | /\d{18,20}/    | /2017\/06\/16 \d\d:09:08/ | 16313739661670634666L | /standby_\d{3}\|\d{20}L/ | 1,000 GCC | 0.1 GCC | Confirmed     |
      | /\d{18,20}/    | /2017\/06\/16 \d\d:09:08/ | 16313739661670634666L | /standby_\d{3}\|\d{20}L/ | 1,000 GCC | 0.1 GCC | Confirmed     |
      | /\d{18,20}/    | /2017\/06\/16 \d\d:09:08/ | 16313739661670634666L | /standby_\d{3}\|\d{20}L/ | 1,000 GCC | 0.1 GCC | Confirmed     |
